...
IF OBJECT_ID('[dbo].[#UpdatedCustomers]', 'U') IS NOT NULL
DROP TABLE [dbo].[#UpdatedCustomers]
GO
...
-- Edited: Replaced erroneous WHERE clause (bad assumption) with clause matching update conditions
...
SELECT DISTINCT * FROM #UpdatedCustomers;
GO
...
-- Edit: Added filter on column name to WHERE clause, added preceding DROP statement

DROP INDEX IF EXISTS Orders.IX_Orders_SaleDate;
GO
...
FROM sys.indexes i LEFT JOIN sys.objects o ON i.[object_id] = o.[object_id]
    LEFT JOIN sys.columns c ON i.[object_id] = c.[object_id]
WHERE i.name = 'IX_Orders_SaleDate' AND c.name = 'sale_date'; -- Is the index supposed to be linked to all columns, or is that simply an issue with the join?
GO
...
-- EDIT: Added TOP 1 and ORDER BY clause to isolate top store

SELECT TOP 1 s.store_name, AVG(o.total_amt) AS avg_sale_amount
FROM Stores s LEFT JOIN Orders o ON s.store_id = o.store_id
GROUP BY s.store_name
...
-- EDIT: Added missing ORDER BY clause
...
-- EDIT: Added unspecified ORDER BY clause
